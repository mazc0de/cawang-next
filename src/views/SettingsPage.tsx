...
export function SettingsPage() {
  ...
  return (
    <div className="space-y-8 pb-12 max-w-4xl mx-auto w-full">
      ...
      <div id="card-profile" className="card-neubrutalism bg-white p-6 sm:p-7 space-y-6">
        <div className="flex items-center gap-3.5 border-b-2 border-ink pb-4">
          ...
          <div>
            ...
            <p className="font-space-grotesk text-xs text-ink/70 mt-0.5">Informasi identitas akun CAWANG Anda</p>
          </div>
        </div>
        ...
        <div className="space-y-4 max-w-xl">
          <div className="space-y-1.5">
            ...
            />
            <p className="text-[11px] font-space-grotesk text-ink/60">Alamat email terdaftar dan tidak dapat diubah.</p>
          </div>

          <div className="space-y-1.5">
            <label htmlFor="settings-name" className="text-xs font-space-grotesk font-bold uppercase tracking-wider text-ink">
              Nama Lengkap
            </label>
            <Input
              id="settings-name"
      ...
      <div id="card-cycle-config" className="card-neubrutalism bg-white p-6 sm:p-7 space-y-6">
        <div className="flex items-center gap-3.5 border-b-2 border-ink pb-4">
          ...
          <div>
            <h3 className="font-archivo-black text-xl text-ink">Siklus Finansial (Financial Cycle)</h3>
            <p className="font-space-grotesk text-xs text-ink/70 mt-0.5">Pengaturan periode tanggal gajian & siklus pembukuan</p>
          </div>
        </div>

        <div className="space-y-5 max-w-xl">
          <div className="p-4 rounded-[14px] bg-canvas border-2 border-ink shadow-hard-sm flex items-start gap-3">
            <Info className="h-5 w-5 text-ink shrink-0 mt-0.5" strokeWidth={2.5} />
            <p className="font-space-grotesk text-xs text-ink/80 leading-relaxed">
              Budget bulanan, grafik analytics, dan perputaran arus kas akan dihitung dimulai dari tanggal siklus yang Anda pilih (misal: <strong>Tanggal {startDay}</strong> sampai <strong>Tanggal {Number(startDay) === 1 ? 'akhir bulan' : Number(startDay) - 1}</strong> bulan berikutnya).
            </p>
          </div>

          <div className="space-y-1.5">
            <label htmlFor="settings-start-day" className="text-xs font-space-grotesk font-bold uppercase tracking-wider text-ink">
              Tanggal Mulai Siklus (Start Day)
            </label>
            <Select value={startDay} onValueChange={setStartDay}>
              <SelectTrigger id="settings-start-day" className="w-56 h-11 text-xs font-space-grotesk font-bold">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                {Array.from({ length: 28 }, (_, i) => i + 1).map((d) => (
                  <SelectItem key={d} value={String(d)} className="font-space-grotesk text-xs font-bold">
                    Tanggal {d} {d === 1 ? '(Awal Bulan)' : d === 25 ? '(Umum Gajian)' : ''}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
      ...
      <div id="card-danger-zone" className="card-neubrutalism bg-white p-6 sm:p-7 space-y-4">
        <div className="flex items-center gap-3.5 border-b-2 border-ink pb-4">
          ...
          <div>
            <h3 className="font-archivo-black text-xl text-coral">Keluar dari Sesi</h3>
            <p className="font-space-grotesk text-xs text-ink/70 mt-0.5">Keluar dari akun Anda pada perangkat ini</p>
          </div>
        </div>

        <div className="flex flex-wrap items-center justify-between gap-4 pt-2">
          <div>
            <p className="font-archivo-black text-sm text-ink">Akhiri Sesi Login</p>
            <p className="font-space-grotesk text-xs text-ink/60 mt-0.5">Anda akan diarahkan kembali ke halaman autentikasi.</p>
          </div>
