...
export function AccountsPage() {
  ...
  const handleDelete = async (account: AccountWithBalance) => {
    if (!confirm(`Hapus Akun "${account.name}"? Semua transaksi terkait akan ikut terhapus.`)) return;
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        ...
        <div className="card-neubrutalism bg-white p-5 flex items-center gap-4 group hover:-translate-y-1 transition-transform md:col-span-2">
          ...
          <div className="min-w-0 flex-1">
            <p className="font-space-grotesk font-bold text-xs uppercase tracking-wider text-ink/70">Total Saldo Semua Akun (Net Worth)</p>
        ...
        <div className="card-neubrutalism bg-canary p-5 flex items-center justify-between gap-4">
          <div>
            <p className="font-archivo-black text-lg text-ink leading-tight">Akun Baru?</p>
            <p className="font-space-grotesk font-medium text-xs text-ink/70 mt-0.5">Tambah bank atau dompet digital</p>
          </div>
          <button
            id="btn-add-account"
      ...
      <div>
        <div className="flex justify-between items-center mb-6">
          <div className="flex items-center gap-3">
            <h2 className="font-archivo-black text-xl text-ink">Daftar Akun & Dompet</h2>
            {!isLoading && (
        ...
        {isLoading && (
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {[1, 2, 3].map((i) => (
              <div key={i} className="h-44 w-full bg-canvas animate-pulse rounded-[18px] border-2 border-ink/10" />
            ))}
          </div>
        )}
        ...
        {!isLoading && accounts.length === 0 && (
          <div className="flex flex-col items-center justify-center py-16 px-4 text-center rounded-[18px] bg-canvas border-2 border-dashed border-ink/20">
            ...
            <h3 className="font-archivo-black text-lg text-ink">Belum Ada Akun</h3>
            <p className="font-space-grotesk text-sm text-ink/70 max-w-sm mt-1 mb-6">
              Tambahkan akun rekening bank, e-wallet, atau kas tunai pertama Anda untuk mulai mencatat keuangan.
            </p>
            <button
