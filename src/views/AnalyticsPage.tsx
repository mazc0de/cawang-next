...
const CustomTooltip = ({ active, payload, label }: any) => {
  ...
  return (
    <div className="bg-white border-2 border-ink rounded-xl p-3 shadow-hard-sm font-space-grotesk text-xs space-y-1">
      {label && (
        <p className="font-archivo-black text-ink text-sm mb-2">{label}</p>
      )}
      {payload.map((p: any, i: number) => (
        <div
          key={i}
          className="flex items-center gap-2 font-space-mono font-bold text-ink"
        >
          <div
            className="h-3 w-3 rounded-full border border-ink"
            style={{ backgroundColor: p.color || p.fill }}
          />
          <span className="text-ink/70 font-space-grotesk">
            {p.name === "income"
              ? "Pemasukan"
              : p.name === "expense"
                ? "Pengeluaran"
...
const PieTooltip = ({ active, payload }: any) => {
  ...
  return (
    <div className="bg-white border-2 border-ink rounded-xl p-3 shadow-hard-sm font-space-grotesk text-xs">
      ...
      <p className="font-space-mono font-bold text-ink mt-1">
...
export function AnalyticsPage() {
  ...
  const { data: analytics, isLoading: isLoadingAnalytics } = useAnalyticsData(
    cycleStart,
    cycleEnd,
  );
  ...
  const totalSpending =
    analytics?.spendingByCategory?.reduce((sum, c) => sum + c.value, 0) || 0;
        ...
        <div className="space-y-6">
          <div className="grid gap-5 sm:grid-cols-3">
            {[1, 2, 3].map((i) => (
              <div
                key={i}
                className="h-28 w-full bg-canvas animate-pulse rounded-[18px] border-2 border-ink/10"
              />
            ))}
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
            ...
            <div className="card-neubrutalism bg-white p-5 flex items-center gap-4 group hover:-translate-y-1 transition-transform">
              ...
              <div className="min-w-0">
                ...
                <p className="font-archivo-black text-2xl sm:text-3xl text-ink tracking-tight truncate mt-0.5">
                  {formatRupiah(stats?.net_worth || 0, true)}
                </p>
              </div>
            </div>
            ...
            <div className="card-neubrutalism bg-white p-5 flex items-center gap-4 group hover:-translate-y-1 transition-transform">
              ...
              <div className="min-w-0">
                ...
                <p className="font-archivo-black text-2xl sm:text-3xl text-ink tracking-tight truncate mt-0.5">
                  {formatRupiah(stats?.income_this_cycle || 0, true)}
                </p>
              </div>
            </div>
            ...
            <div className="card-neubrutalism bg-white p-5 flex items-center gap-4 group hover:-translate-y-1 transition-transform">
              ...
              <div className="min-w-0">
                ...
                <p className="font-archivo-black text-2xl sm:text-3xl text-ink tracking-tight truncate mt-0.5">
                  {formatRupiah(stats?.expense_this_cycle || 0, true)}
                </p>
              </div>
            </div>
          </div>
                  ...
                  <div>
                    <div className="flex justify-between items-start mb-4">
                      <div>
                        ...
                        <p className="font-space-grotesk text-xs text-ink/60 mt-0.5">
                          Total pengeluaran cycle ini:{" "}
                          <span className="font-space-mono font-bold text-ink">
                    ...
                    analytics.spendingByCategory.length > 0 ? (
                        ...
                        <ResponsiveContainer width="100%" height="100%">
                          <PieChart>
                            <Pie
                              ...
                              strokeWidth={2}
                            >
                              {analytics.spendingByCategory.map(
                                (entry, index) => (
                                  <Cell
                                    key={index}
                                    fill={
                                      entry.color ||
                                      NEUBRUTALISM_PALETTE[
                                        index % NEUBRUTALISM_PALETTE.length
                                      ]
                                    }
                                  />
                                ),
                      ...
                      <div className="flex h-[280px] flex-col items-center justify-center text-center rounded-[14px] bg-canvas border-2 border-dashed border-ink/20">
                        <p className="font-space-grotesk font-medium text-sm text-ink/60">
                          Belum ada pengeluaran pada cycle ini
                        </p>
                      </div>
                    )}
                  </div>
                      ...
                      <div className="space-y-4">
                        {analytics.spendingByCategory.map((cat, idx) => {
                          ...
                          const catColor =
                            cat.color ||
                            NEUBRUTALISM_PALETTE[
                              idx % NEUBRUTALISM_PALETTE.length
                            ];

                          return (
                            <div
                              key={cat.name}
                              className="p-3.5 rounded-[14px] bg-white border-2 border-ink shadow-hard-sm"
                            >
                              <div className="flex items-center justify-between mb-2">
                                <div className="flex items-center gap-2.5 min-w-0">
                                  ...
                                  />
                                  <span className="font-space-grotesk font-bold text-sm text-ink truncate">
                                    {cat.name}
                                  </span>
                                </div>
                                <div className="flex items-center gap-2 shrink-0">
                                  <span className="font-space-mono font-bold text-xs text-ink">
                                    {formatRupiah(cat.value)}
                                  </span>
                                  <span className="px-2 py-0.5 rounded-full bg-canary border border-ink font-space-mono text-[10px] font-bold text-ink">
                                    {pct.toFixed(0)}%
                                  </span>
                                </div>
                              </div>
              ...
              <div className="card-neubrutalism bg-white p-6">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 mb-6">
                  <div>
                    ...
                    <p className="font-space-grotesk text-xs text-ink/60 mt-0.5">
                      Perbandingan data dalam 5 bulan terakhir
                    </p>
                  </div>
                ...
                {analytics?.incomeExpenseByMonth &&
                analytics.incomeExpenseByMonth.length > 0 ? (
                  <div className="h-[340px] w-full">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart
                        data={analytics.incomeExpenseByMonth}
              ...
              <div className="card-neubrutalism bg-white p-6">
                <div className="flex justify-between items-start mb-6">
                  <div>
                    ...
                    <p className="font-space-grotesk text-xs text-ink/60 mt-0.5">
                      Pertumbuhan total saldo seluruh akun
                    </p>
                  </div>
                </div>

                {analytics?.netWorthOverTime &&
                analytics.netWorthOverTime.length > 0 ? (
                  <div className="h-[340px] w-full">
                    <ResponsiveContainer width="100%" height="100%">
                      <LineChart
                        data={analytics.netWorthOverTime}
              ...
              <div className="card-neubrutalism bg-white p-6">
                <div className="flex justify-between items-start mb-6">
                  <div>
                    ...
                    <p className="font-space-grotesk text-xs text-ink/60 mt-0.5">
                      Perbandingan alokasi anggaran dengan realisasi pengeluaran
                    </p>
                  </div>
                </div>
                ...
                {budgetVsActual.length > 0 ? (
                  <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-6">
                      {budgetVsActual.map((b) => {
                        ...
                        const pct =
                          b.budget > 0 ? (b.actual / b.budget) * 100 : 0;
                        return (
                          <div
                            key={b.category}
                            className="p-4 rounded-[14px] bg-white border-2 border-ink shadow-hard-sm flex items-center justify-between gap-3"
                          >
                            <div className="min-w-0">
                  ...
                  <div className="flex h-[300px] flex-col items-center justify-center text-center rounded-[14px] bg-canvas border-2 border-dashed border-ink/20">
                    <p className="font-space-grotesk font-medium text-sm text-ink/60">
                      Belum ada data anggaran pada periode ini
                    </p>
                  </div>
                )}
              </div>
