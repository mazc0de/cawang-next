...
export function TransactionFormDialog({
  ...
  useEffect(() => {
    if (open && transaction) {
      const type = transaction.transfer_pair_id
        ? "transfer"
        : (transaction.type as "inflow" | "outflow");
      reset({
        type,
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[425px]" id="transaction-form-dialog">
        ...
        >
            ...
            >
              <TabsList className="grid w-full grid-cols-3">
                <TabsTrigger
                  value="outflow"
                  id="tab-outflow"
                  className="data-[state=active]:text-expense"
                >
                  Pengeluaran
                </TabsTrigger>
                <TabsTrigger
                  value="inflow"
                  id="tab-inflow"
                  className="data-[state=active]:text-income"
                >
                  Pemasukan
                </TabsTrigger>
                <TabsTrigger
          ...
          {/* From account */}
          <FieldWrapper
            label={type === "transfer" ? "Dari Account" : "Account"}
          ...
          <div className="pt-4 flex justify-end gap-3">
            <button
              type="button"
              className="btn-neubrutalism bg-white px-5 py-2 text-sm font-space-grotesk text-ink"
              id="btn-cancel"
            ...
            <button
              type="submit"
              className="btn-neubrutalism bg-hot-pink px-5 py-2 text-sm font-space-grotesk text-ink"
              id="btn-submit"
            >
              Simpan
            </button>
