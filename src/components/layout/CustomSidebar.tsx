    ...
    <div className="flex h-full w-[240px] flex-col border-r-2 border-ink bg-white shrink-0 z-20 overflow-hidden relative">
      ...
      <div className="flex items-center gap-3 px-5 py-6 border-b-2">
        ...
        <div className="flex flex-col min-w-0">
          ...
          <span className="font-space-grotesk text-[11px] font-bold text-ink/70 leading-tight mt-1 truncate">
            Catat Keuangan
          </span>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-6 space-y-2">
        {mainNavItems.map((item) => {
          ...
          return (
            ...
            >
              <Icon
                className={cn("size-5", isActive ? "fill-ink/20" : "")}
                strokeWidth={isActive ? 2.5 : 2}
              />
              <span className="text-[15px] font-space-grotesk">
                {item.label}
              </span>
            </Link>
          );
