...
const COLORS = [
  '#f97316', '#3b82f6', '#a855f7', '#ec4899', '#64748b',
  '#ef4444', '#14b8a6', '#84cc16', '#facc15', '#0ea5e9'
]
...
export function useAnalyticsData(cycleStart: Date, cycleEnd: Date) {
  ...
  return useQuery({
    queryKey: ['analytics_data', user?.id, cycleStart.toISOString(), cycleEnd.toISOString()],
    queryFn: async () => {
      if (!user) return null
      ...
      const spendingByCategory: SpendingByCategory[] = Object.values(spendingMap)
      ...
      const months: { year: number, month: number, label: string, dateStr: string, income: number, expense: number, netWorth: number }[] = []
      for (let i = 4; i >= 0; i--) {
      ...
      const fiveMonthsAgoStart = new Date(currentYear, currentMonth - 4, 1).toISOString().split('T')[0]
      ...
      historicalTx.forEach(tx => {
        ...
        const monthEntry = months.find(m => m.year === year && m.month === month)
        if (monthEntry) {
