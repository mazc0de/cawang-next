...
export function useTransactions(filters?: TransactionFilters) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      let query = supabase
        .from('transactions')
        .select('*, account:accounts(id,name,type), category:categories(id,name,icon,color,type), transaction_tags(tag:tags(id,name))')
        .eq('user_id', user.id)
...
export function useCreateTransaction() {
  ...
  return useMutation({
    mutationFn: async (newTransaction: Omit<Transaction, 'id' | 'user_id' | 'created_at' | 'updated_at' | 'account' | 'category' | 'tags'>) => {
